...
const MOCK_CLIPS = [
  { id: 1, user: { name: "Maya Chen", handle: "@mayachen", avatar: "MC", color: "#FF6B6B" }, title: "sunset kayaking in Ha Long Bay 🌅", likes: 4821, comments: 312, shares: 89, views: "128K", tags: ["travel", "vietnam", "adventure"], gradient: "linear-gradient(135deg, #FF6B6B 0%, #FFE66D 50%, #4ECDC4 100%)", duration: 90 },
  { id: 2, user: { name: "Kai Rivers", handle: "@kaimusic", avatar: "KR", color: "#A29BFE" }, title: "wrote this beat in 90 seconds 🎧", likes: 9204, comments: 671, shares: 443, views: "512K", tags: ["music", "beatmaking", "producer"], gradient: "linear-gradient(135deg, #A29BFE 0%, #6C5CE7 50%, #2D3436 100%)", duration: 90 },
  { id: 3, user: { name: "Zoe Park", handle: "@zoecooks", avatar: "ZP", color: "#55EFC4" }, title: "90-sec ramen from scratch 🍜", likes: 31540, comments: 2109, shares: 1880, views: "2.1M", tags: ["food", "cooking", "ramen"], gradient: "linear-gradient(135deg, #55EFC4 0%, #00B894 50%, #FDCB6E 100%)", duration: 90 },
  { id: 4, user: { name: "Leo Vance", handle: "@leoskates", avatar: "LV", color: "#FD79A8" }, title: "first time landing this trick 🛹", likes: 7633, comments: 890, shares: 210, views: "890K", tags: ["skate", "tricks", "sports"], gradient: "linear-gradient(135deg, #FD79A8 0%, #E84393 50%, #6C5CE7 100%)", duration: 90 },
];

const TABS = ["Feed", "Explore", "Upload", "Notifications", "Profile"];

function ClipCard({ clip, active, onLike, liked }) {
      ...
      <div style={{ background: "linear-gradient(to top, rgba(0,0,0,0.85), transparent)", padding: "60px 18px 18px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 36, height: 36, borderRadius: "50%", background: clip.user.color, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: 12, color: "#fff", border: "2px solid rgba(255,255,255,0.4)" }}>{clip.user.avatar}</div>
            <div>
          ...
          <button style={{ background: "rgba(255,255,255,0.15)", border: "1px solid rgba(255,255,255,0.3)", color: "#fff", borderRadius: 20, padding: "6px 16px", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>Follow</button>
        </div>
      </div>
    </div>
  );
...
function ActionBar({ clip, liked, onLike }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-around", padding: "14px 0 4px", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
      {[{ icon: liked ? "❤️" : "🤍", label: clip.likes.toLocaleString(), action: onLike }, { icon: "💬", label: clip.comments.toLocaleString(), action: () => {} }, { icon: "↗️", label: clip.shares.toLocaleString(), action: () => {} }, { icon: "👁", label: clip.views, action: () => {} }].map(({ icon, label, action }) => (
    ...
    <div style={{ minHeight: "100vh", background: "#0A0A0F", color: "#fff", fontFamily: "sans-serif", maxWidth: 420, margin: "0 auto", display: "flex", flexDirection: "column" }}>
      ...
      {activeTab === "Feed" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 16px 100px" }}>
          {MOCK_CLIPS.map((clip, i) => (
            <div key={clip.id} onClick={() => setActiveClip(i)} style={{ marginBottom: 20, borderRadius: 20, overflow: "hidden", background: "rgba(255,255,255,0.03)", border: `1px solid ${activeClip === i ? "rgba(255,107,107,0.4)" : "rgba(255,255,255,0.06)"}` }}>
              <ClipCard clip={clip} active={activeClip === i} liked={likedClips.has(clip.id)} onLike={() => toggleLike(clip.id)} />
      ...
      {activeTab === "Upload" && (
        <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 32, gap: 20, textAlign: "center" }}>
          ...
          <a href="/upload" style={{ background: "linear-gradient(135deg, #FF6B6B, #A29BFE)", borderRadius: 16, padding: "14px 36px", fontSize: 15, fontWeight: 700, cursor: "pointer", color: "#fff", textDecoration: "none" }}>Choose Video</a>
        </div>
      )}
